...
import { debtData, systemNodes, aggregatedRiskScore, coverageGap } from '../../data/debt'
...
export function TechDebtPanel() {
  ...
  return (
    <div className="h-full overflow-y-auto pb-16 transition-theme" style={{ backgroundColor: 'var(--bg-app)' }}>
      <div className="max-w-6xl mx-auto p-8 space-y-8">
        {/* Header with aggregated risk score */}
        <header className="flex items-end justify-between">
          <div>
            <h2 className="text-4xl font-bold font-display tracking-tighter" style={{ color: 'var(--text-primary)' }}>
              Tech Debt Heatmap
            </h2>
            <p className="text-sm mt-2 max-w-xl" style={{ color: 'var(--text-secondary)' }}>
              Real-time architectural risk analysis across neural sub-systems.
            </p>
          </div>
          <div className="flex items-center gap-6 glass p-5 rounded-xl">
            <div className="text-right">
              <p className="font-display text-[10px] uppercase tracking-widest" style={{ color: 'var(--neon-magenta)' }}>
                Total Risk Score
              </p>
              <p className="text-3xl font-display font-bold mt-1" style={{ color: 'var(--text-primary)' }}>
                {aggregatedRiskScore}<span className="text-sm ml-1" style={{ color: 'var(--neon-magenta)' }}>/10</span>
              </p>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="font-display text-[10px] uppercase tracking-widest" style={{ color: 'var(--neon-error)' }}>
                  Coverage Gap
                </p>
                <p className="text-3xl font-display font-bold mt-1" style={{ color: 'var(--neon-error)' }}>
                  {coverageGap}%
                </p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          {primaryDebt && (
            <section className="col-span-12 lg:col-span-7">
              <div className="glass p-7 rounded-xl relative overflow-hidden group hover:bg-[var(--surface-card-hover)] transition-all duration-300">
                ...
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <span className="inline-block px-3 py-1 text-[10px] font-display font-bold uppercase tracking-widest rounded-sm mb-3" style={{ backgroundColor: 'rgba(159, 5, 25, 0.8)', color: '#ffa8a3' }}>
                      CRITICAL_VULNERABILITY
                    </span>
                    <h3 className="text-2xl font-display font-bold tracking-tight" style={{ color: 'var(--text-primary)' }}>
                      {primaryDebt.title}
                    </h3>
                  </div>
                  <div className="text-right">
                    <p className="text-3xl font-display font-bold" style={{ color: 'var(--neon-error)' }}>{primaryDebt.riskScore}</p>
                    <p className="text-[10px] uppercase font-display" style={{ color: 'var(--text-muted)' }}>Risk Score</p>
                  </div>
                </div>
                {/* Severity gauges */}
                ...
                {primaryDebt.details && (
                  <div className="grid grid-cols-3 gap-4 mb-6">
                    ...
                    <MetricChip label="Cyclomatic Complexity" value={String(primaryDebt.details.complexity ?? '—')} accent="error" />
                    <MetricChip label="Coupled Modules" value={String(primaryDebt.details.coupledModules ?? '—')} />
                  </div>
                )}
                <div className="flex items-center gap-3 p-3 rounded-lg" style={{ backgroundColor: 'rgba(255, 113, 108, 0.06)', border: '1px solid rgba(255, 113, 108, 0.1)' }}>
                  <AlertTriangle size={16} style={{ color: 'var(--neon-error)' }} />
                  <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                    Cyclomatic complexity exceeds threshold by <span className="font-bold" style={{ color: 'var(--neon-error)' }}>420%</span>. Circular dependency detected.
                  </p>
                </div>
              </div>
            </section>
          ...
          {secondaryDebt && (
            <section className="col-span-12 lg:col-span-5">
              <div className="glass p-7 rounded-xl flex flex-col h-full hover:bg-[var(--surface-card-hover)] transition-all duration-300">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <span className="inline-block px-3 py-1 text-[10px] font-display font-bold uppercase tracking-widest rounded-sm mb-3" style={{ backgroundColor: 'rgba(169, 0, 169, 0.6)', color: '#fff5f9' }}>
                      DEPRECATED_SYSTEM
                    </span>
                    <h3 className="text-2xl font-display font-bold tracking-tight" style={{ color: 'var(--text-primary)' }}>
                      Legacy API Gateway
                  ...
                  <div className="text-right">
                    <p className="text-3xl font-display font-bold" style={{ color: 'var(--neon-magenta)' }}>{secondaryDebt.riskScore}</p>
                    <p className="text-[10px] uppercase font-display" style={{ color: 'var(--text-muted)' }}>Risk Score</p>
                  </div>
                </div>
                <div className="flex-1 space-y-3">
                  {secondaryDebt.details && Object.entries(secondaryDebt.details).map(([key, val]) => (
                    <div key={key} className="flex justify-between items-center py-2" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                ...
                <p className="text-sm mt-4" style={{ color: 'var(--text-secondary)' }}>{secondaryDebt.desc}</p>
              </div>
            </section>
          )}
        </div>
        ...
        <section className="glass p-7 rounded-xl">
          <div className="flex justify-between items-center mb-6">
            <h4 className="font-display text-lg font-bold uppercase tracking-tight" style={{ color: 'var(--text-primary)' }}>
          ...
          <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {systemNodes.map((node) => {
              ...
              return (
                <div
                  key={node.id}
                  className="h-20 rounded-lg flex flex-col items-center justify-center p-3 cursor-pointer transition-all hover:scale-[1.03]"
                  style={{
                    backgroundColor: colors.bg,
                    border: `1px solid ${colors.border}`,
                  }}
                >
                  <p className="text-[10px] font-display font-bold mb-1" style={{ color: colors.text }}>{node.label}</p>
                  <p className="text-base font-display font-bold" style={{ color: 'var(--text-primary)' }}>{node.risk}%</p>
                </div>
              )
            })}
          </div>
...
const statusColors: Record<string, { bg: string; border: string; text: string }> = {
  critical: { bg: 'rgba(255, 113, 108, 0.12)', border: 'rgba(255, 113, 108, 0.3)', text: 'var(--neon-error)' },
  high: { bg: 'rgba(255, 81, 250, 0.08)', border: 'rgba(255, 81, 250, 0.2)', text: 'var(--neon-magenta)' },
  moderate: { bg: 'rgba(234, 179, 8, 0.06)', border: 'rgba(234, 179, 8, 0.15)', text: 'rgb(234, 179, 8)' },
  optimal: { bg: 'rgba(142, 255, 113, 0.06)', border: 'rgba(142, 255, 113, 0.15)', text: 'var(--neon-lime)' },
}
...
function GaugeCard({ label, value, color, percent }: { label: string; value: string; color: string; percent: number }) {
  ...
  return (
    <div className="glass p-4 rounded-lg">
      ...
      <p className="text-lg font-display font-bold" style={{ color: barColor }}>{value}</p>
